...
set.seed(param_seed)
sound <- cbind(
  sample(freq, size=param_examples, replace=TRUE),
  round(runif(param_examples, min=min_dB, max=max_dB), digits=digits)
)
...
for (row in 1:nrow(sound)) {
  ...
  p <- sound[row, 2]
  idx <- which(freq == f)
  print(stringr::str_interp("A sound (${f} Hz, ${p} dB), is ${round(p + 40 - reshaped_results[['40']][[idx]], digits=digits)} dBA, ${round(p + 70 - reshaped_results[['70']][[idx]], digits=digits)} dBB, ${round(p + 100 - reshaped_results[['100']][[idx]], digits=digits)} dBC, and ${round(L_N(p, alpha_f(f), L_U(f), T_f(f)), digits=digits)} Phon."))
}
```
